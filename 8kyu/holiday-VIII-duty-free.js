...
function dutyFree(normPrice, discount, hol){
    let discountedPrice = normPrice - (normPrice * (discount/100)) // Discounted price
    console.log("Discounted price: ",discountedPrice)
    let savedAmount = normPrice-discountedPrice // The amount we save on each bottle
    console.log("Saved amount: ",savedAmount)
    let bottlesNeeded = hol/savedAmount // The amount of bottles we need to compensate for the holiday
    let bottlesNeededRounded = Math.floor(bottlesNeeded) //Rounding down
    console.log("Bottles needed: ",bottlesNeeded)
    console.log(bottlesNeededRounded)
    return bottlesNeededRounded
}
dutyFree(12,50,1000)